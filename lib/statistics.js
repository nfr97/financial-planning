 ...
 * @param {boolean} [options.sorted=false] - If true, skip sorting (data is already sorted ascending)
 * @returns {number} Value at the given percentile
 */
export function getPercentile(data, percentile, options) {
  if (!data || data.length === 0) return 0;

  const sorted = options && options.sorted ? data : [...data].sort((a, b) => a - b);
