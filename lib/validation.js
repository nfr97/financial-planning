...
export function validateRetirementParams(params) {
  ...
  const LIMITS = {
    ...
    traditionalIRA: 7000,
    traditionalIRACatchUp: 1000,
  };

  if (params.contributions) {
    const catchUpEligible = currentAge >= 50;
    const max401k = LIMITS.traditional401k + (catchUpEligible ? LIMITS.traditional401kCatchUp : 0);
    const maxIRA = LIMITS.traditionalIRA + (catchUpEligible ? LIMITS.traditionalIRACatchUp : 0);
